...
SERVICES = {
    ...
    "network-mcp": Service(
        name="network-mcp",
        command=[".venv/bin/python", "mcp_servers/network_mcp/main.py"],
        cwd=ROOT,
        port_env="NETWORK_MCP_PORT",
        default_port=8101,
        required_path=ROOT / "mcp_servers" / "network_mcp" / "main.py",
    ),
    "customer-mcp": Service(
        name="customer-mcp",
        command=[".venv/bin/python", "mcp_servers/customer_mcp/main.py"],
        cwd=ROOT,
        port_env="CUSTOMER_MCP_PORT",
        default_port=8102,
        required_path=ROOT / "mcp_servers" / "customer_mcp" / "main.py",
    ),
    "rule-mcp": Service(
        name="rule-mcp",
        command=[".venv/bin/python", "mcp_servers/rule_mcp/main.py"],
        cwd=ROOT,
        port_env="RULE_MCP_PORT",
        default_port=8103,
        required_path=ROOT / "mcp_servers" / "rule_mcp" / "main.py",
    ),
    "compensation-mcp": Service(
        name="compensation-mcp",
        command=[".venv/bin/python", "mcp_servers/compensation_mcp/main.py"],
        cwd=ROOT,
        port_env="COMPENSATION_MCP_PORT",
        default_port=8104,
        required_path=ROOT / "mcp_servers" / "compensation_mcp" / "main.py",
    ),
