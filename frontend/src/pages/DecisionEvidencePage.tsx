...
function GeneralEvidenceDetailView({ detail }: { detail: EvidenceRecordDetail }) {
  return <>
    ...
    {detail.tool_timeline.length ? <section className="evidence-detail__section"><h2>Tool zaman çizelgesi</h2><ol className="evidence-detail__timeline">{detail.tool_timeline.map((tool) => <li key={tool.sequence}><div className="evidence-detail__timeline-header"><strong>{tool.sequence}. {tool.tool_name}</strong><span>{tool.status}</span></div><dl><dt>MCP/server</dt><dd>{tool.server}</dd><dt>Deneme</dt><dd>{tool.attempt_count ?? 'Belirtilmedi'}</dd><dt>Süre</dt><dd>{tool.duration_ms === null ? 'Belirtilmedi' : `${tool.duration_ms} ms`}</dd>{tool.result_summary ? <><dt>Güvenli sonuç özeti</dt><dd>{tool.result_summary}</dd></> : null}{tool.error_code ? <><dt>Hata kodu</dt><dd>{tool.error_code}</dd></> : null}</dl></li>)}</ol></section> : null}
