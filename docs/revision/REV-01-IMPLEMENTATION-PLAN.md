...
| REV-10 — Dört MCP uyarlaması | **Tamamlandı.** Causal analysis kanıtlarını mevcut MCP araçlarına additive yansıtmak | Network/Customer/Rule/Compensation causal public-code mapping | `mcp_servers` | tool unit ve shared contracts | Tool sayıları 9/7/8/6 korunur; raw/PII sızmaz | `feat: expose causal analysis through mcp tools` |
