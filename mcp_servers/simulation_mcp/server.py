...
SERVER_NAME = "processtwin-simulation"


def build_server(tools: SimulationMCPTools) -> Server:
    async def list_tools(_ctx, _params) -> types.ListToolsResult:
        return types.ListToolsResult(
            tools=[
                ...
                for definition in SIMULATION_TOOL_DEFINITIONS.values()
            ]
        )

    async def call_tool(_ctx, params: types.CallToolRequestParams) -> types.CallToolResult:
        response = await anyio.to_thread.run_sync(tools.run, params.name, params.arguments or {})
    ...
    return Server(
        SERVER_NAME,
        version=SIMULATION_MCP_VERSION,
...
async def run_stdio() -> None:
    client = InternalAPIClient(config=InternalAPIClientConfig.from_env())
    server = build_server(SimulationMCPTools(client))
